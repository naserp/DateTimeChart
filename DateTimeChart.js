function DateTimeChart(width, height, marginx, marginy,tricky) {
	...
	this.setDataMonth = function(d, dOrigin, labels, mStep, mPage) {
		this.data = d;
		this.dataOrigin = dOrigin;
		this.axisLabels = labels;
		this.monthStep = mStep;
		this.monthPage = mPage;
		this.paintMonth();
	};    	
	...
	this.paintMonth = function() {
		...
		for (var i = 0; i < 31; i+=3) {
        	...
        	g.append("svg:text").attr("class", "xLabel").text(this.axisLabels[i/3]).attr("x", positionX).attr("y", -1 * y(0) + 20).attr("text-anchor", "middle");
        }
	
    	var indexlabel = this.monthStep;
    	var sw = this.monthPage % 2;
    	var color;
    	if(sw == 0)
    		color = "steelblue";
    	else 
